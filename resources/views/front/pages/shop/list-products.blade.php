<ul class="products-list product-hover-default">
    @forelse ($products->items as $item)
        <li
            class="product-efect-scale product-list product type-product post-271 status-publish first instock product_cat-allium product_tag-bread has-post-thumbnail shipping-taxable purchasable product-type-variable">
            <div class="product-image">
                <img width="450" height="420" src="{{ $item->url_image }}"
                    class="attachment-woocommerce_thumbnail lazyloaded size-woocommerce_thumbnail border-blue" alt="{{ $item->name }}" decoding="async">
            ...
            <div class="product-caption">
                <h3 class="woocommerce-loop-product__title title-product-redisign">
                    <a class="item-title-link" href="{{ route('front.shop.show', $item->id) }}">{{ $item->name }}</a>
                </h3>
                ...
                <span class="price">
                    ...
                    @endif
                    <ins><span class="woocommerce-Price-amount amount style-total-redisign"><bdi><span class="woocommerce-Price-currencySymbol">$</span>{{ $item->price_format }}</bdi></span></ins>
                </span>
